...
async function travelTo(locationId) {
  ...
  result.textContent = `喵呜沿着通往${destination.name}的布路线出发了…`;
  try {
...
function updateVoice(payload) { state.voice = payload; const available = payload && !payload.error && payload.status !== 'unavailable'; const pill = $('#voice-pill'); pill.className = `mini-pill ${available ? 'ok' : 'muted'}`; pill.textContent = available ? '声音已醒' : '还没接上'; setText('#voice-profile', available ? '喵呜的当前声线' : '喵呜的第一种声线'); setText('#voice-description', available ? '声音已经准备好；等有输出设备时就能听见它。' : '声音还在等一副合适的耳朵。'); $('#voice-raw').textContent = JSON.stringify(payload || { status: 'not checked' }, null, 2); }
