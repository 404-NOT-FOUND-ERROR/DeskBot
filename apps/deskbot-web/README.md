...
- “角色方向试行”在研究视图中读取 `fantasy-pull` 候选和服务端提案记录；可以提出方向、选择 `try/later/reject`、记录有限窗口内的正负反馈并明确确认/回退/归档。活动试行会在页面和聊天回合中显示当前方向覆盖，并通过 `expression_intent.v1` 传递给文字、屏幕和未来 TTS；页面只调用 `/api/roles/*`，接受方向不会自动改 Soul 或换壳。
